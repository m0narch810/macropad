...
function csGammaAt(spot: number, strike: number, T: number, vol: number, r: number, q: number, isCall: boolean, skew: number, kurtExcess: number): number {
  ...
  const h = spot * 0.005;
  const v0 = csPrice(spot, strike, T, vol, r, q, isCall, skew, kurtExcess);
  const up = csPrice(spot + h, strike, T, vol, r, q, isCall, skew, kurtExcess);
  const down = csPrice(spot - h, strike, T, vol, r, q, isCall, skew, kurtExcess);
...
function buildHeroStatement(
  ...
  const parts: string[] = [];
  parts.push(`${symbol} is in ${phase.cancellationRatio > 0.6 && phase.phase !== "pinned" ? "fragile " : ""}${phase.label.toLowerCase()} gamma.`);

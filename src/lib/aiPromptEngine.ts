...
export function buildHedgeCliffPrompt(data: GexResponse): string {
  return `You are a 0DTE options hedge-flow analyst focused specifically on gamma cliffs - strikes where dealer hedging would accelerate sharply if spot reaches them, which a naive "current gamma x move size" estimate misses. Use this real data from ${data.symbol}:

${snapshotBlock(data)}

${effectiveGexTable(data, 20)}

${shadowGammaTable(data, 20)}

METHODOLOGY: Effective GEX fully reprices delta at the scenario spot (surface-consistent IV, not frozen) instead of multiplying today's gamma by a fixed move - the Acceleration column flags strikes whose static GEX understates the real hedge response. Shadow Gamma isolates the vanna-driven (IV-surface) component of that response, separate from pure spot-gamma.

TASK: Identify the 1-2 strikes most likely to act as a hedge cliff (where crossing them would force a disproportionate acceleration in dealer hedging) in each direction, and explain in plain terms what a trader should expect to happen (which way price gets pushed, how fast) if spot reaches each one.`;
}

export interface PromptDef {
  id: "master" | "quickbias" | "hedgecliff";
  label: string;
  description: string;
...
export const PROMPTS: PromptDef[] = [
  ...
  { id: "hedgecliff", label: "HEDGE CLIFF", description: "Focused on Effective GEX/Shadow Gamma acceleration only.", build: buildHedgeCliffPrompt },
];

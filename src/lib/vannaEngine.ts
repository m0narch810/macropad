 ...
 *
 * VEX_i = q_i * Vanna_i * OI_i * 100, where Vanna_i is already expressed per
 ...
 * shares of theoretical hedge per 1-vol-point IV move, not dollars.
 *
...
/** d(delta)/d(vol), scaled per 1 full vol point (matches blackScholes.ts bsGreeks' vanna convention). */
function bsVannaAt(spot: number, strike: number, T: number, vol: number, r: number, q: number, isCall: boolean): number {
  if (T <= 0 || vol <= 0.002) return 0;
  const hVol = 0.01;
  const deltaUp = bsDeltaAt(spot, strike, T, vol + hVol, r, q, isCall);
  const deltaDown = bsDeltaAt(spot, strike, T, Math.max(1e-4, vol - hVol), r, q, isCall);
  return (deltaUp - deltaDown) / 2;
}
...
/** Full delta-repricing theoretical hedge, in shares, under a uniform parallel IV shock (ivShift, in vol points e.g. 0.01 = +1pt) applied on top of each contract's own quoted IV. H = -DEX. */
...
function computeIvShockScenarios(chain: ChainStrikeInput[], spot: number, T: number, r: number, q: number, netVex: number, recentVolume15m: number | null): IvShockScenario[] {
  ...
  return shocks.map((pts) => {
    ...
    const linear = -netVex * pts; // netVex per +1 point; hedge change = -VEX*shock

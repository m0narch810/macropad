...
function computeIvStability(chain: ChainStrikeInput[], spot: number, T: number, r: number, q: number, decayDominanceBase: boolean): { scenarios: IvStabilityResult[]; robustnessPct: number } {
  ...
  const scenarios = shifts.map((ivShift) => {
    ...
    const decayStillDominant = extrinsic > 0 ? burn30m / extrinsic > 0.05 === decayDominanceBase : decayDominanceBase;
...
export function computeThetaEngine(params: {
  ...
  const thetaCarry30mDollarPerDay = dealerCarryNow; // per-calendar-day convention, used as the carry baseline for wipeout risk
  const carryWipeoutScenarios = computeCarryWipeoutScenarios(chain, spot, T, r, q, Math.abs(thetaCarry30mDollarPerDay) || 1, emLow, emHigh);

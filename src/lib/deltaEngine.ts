...
function computeExpiryDexStack(crossExpiry: CrossExpiryRow[]): ExpiryDexStack {
  ...
  const rows = crossExpiry.map((row) => ({
    ...
    sharePctNet: (row.netDex / (crossExpiry.reduce((s, r2) => s + r2.netDex, 0) || 1)) * 100,
...
function buildHeroStatement(
  ...
  parts.push(`The option book carries an estimated ${netDex >= 0 ? "+" : ""}${(netDex / 1_000_000).toFixed(1)} million shares of delta, implying a theoretical ${-netDex >= 0 ? "+" : ""}${(-netDex / 1_000_000).toFixed(1)} million-share hedge.`);
...
export function computeDeltaEngine(params: {
  ...
  const diagnostics: DeltaEngineDiagnostics = {
    pricingModel: "Black-Scholes bump-and-reprice delta on SVI-smoothed 0DTE smile (American/CRR trees available via the page's engine toggle for the static table)",

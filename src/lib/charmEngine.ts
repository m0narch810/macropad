 ...
 * CHEX_i(h) = q_i * OI_i * 100 * [Delta_i(T-h) - Delta_i(T)], and the
 * theoretical hedge adjustment is H_Charm(h) = -sum(CHEX_i(h)). Positive
 * H_Charm = modeled dealer buying; negative = modeled dealer selling.
 *
...
/** d(delta)/dT bumped-time, scaled per calendar day (matches blackScholes.ts bsGreeks' charm convention) - used only for the linear-attribution / linearization-risk comparison, never as the headline number. */
function bsCharmAt(spot: number, strike: number, T: number, vol: number, r: number, q: number, isCall: boolean): number {
  // T is in YEARS - 0DTE T is routinely ~2e-5 to ~1e-4 (minutes to hours),
  // so this guard must only catch true near-zero time, not "less than a
  // trading day" (which a threshold like 0.002 - about 17.5 hours -
  // would, zeroing every 0DTE charm value).
  if (T <= 1e-7 || vol <= 0) return 0;
  // T*0.1 collapses to a sub-minute bump for 0DTE (T on the order of
  // 30-120 minutes in years), which is below the finite-difference noise
  // floor of two independently-bumped bsDeltaAt calls - floor it at 1
  // minute so the derivative stays numerically meaningful intraday.
  const hT = Math.max(Math.min(T * 0.1, 1 / 365), 1 / (24 * 60 * 365));
  const delta0 = bsDeltaAt(spot, strike, T, vol, r, q, isCall);
  const deltaBumped = bsDeltaAt(spot, strike, Math.max(1e-6, T - hT), vol, r, q, isCall);
  return ((deltaBumped - delta0) / hT) * (1 / 365);
}

type ChexLadder = { strike: number; chex: number }[];

/** Full delta-repricing theoretical hedge adjustment over horizon h (years), in shares, dealer-sign-weighted: H_Charm(h) = -sum CHEX_i(h). Evaluated at evalSpot so the same function serves both the actual-spot flow schedule and the hypothetical-price field. */
...
/** Static-hedge (no time passage) delta ladder used only for the gamma-implied comparison in the Charm-Gamma Conflict risk. */

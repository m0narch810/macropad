...
function computeColorAdjustedForwardGex(chain: ChainStrikeInput[], spot: number, T: number, r: number, q: number, currentNetGex: number, totalMinutesToExpiry: number): ColorForwardRow[] {
  ...
  return horizons.map((minutesAhead) => {
    ...
    const linearProjection = currentNetGex + totalColorDollar * (minutesAhead / (60 * 24));
...
function computeImpliedMoments(spot: number, T: number, r: number, q: number, sviParams: SviParams, forward: number): ImpliedMoments {
  ...
  for (let i = 1; i < strikes.length - 1; i++) {
    ...
    density.push({ x: Math.log(K / forward), d: q_K });
  }
...
export function computeGexPageAnalytics(params: {
  ...
  const colorForward = computeColorAdjustedForwardGex(chain, spot, T, r, q, totalGex0dte, totalMinutesToExpiry);
